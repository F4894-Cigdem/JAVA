...
public class C06_distinct {
    ...
    private static void ciftKarePrint(List<String> menu) {
        menu.stream().map(String::length).filter(t->t%2==0).map(t->(int)Math.pow(t,2)).sorted(Comparator.reverseOrder()).distinct().forEach(t-> System.out.println(t+" "));
    }
